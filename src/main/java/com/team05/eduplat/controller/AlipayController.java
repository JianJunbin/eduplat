...
import com.team05.eduplat.config.AlipayConfig;
...
public class AlipayController {
     ...
     */
    @PostMapping("/pay")
    public void pay(HttpServletRequest request, HttpServletResponse response, @RequestBody AlipayParam alipayParam) throws Exception {

        // 模拟从前台传来的数据
        String orderNo = alipayParam.getUserId().toString()+alipayParam.getCourseId().toString()+System.currentTimeMillis(); // 生成订单号
        String totalAmount = alipayParam.getPrice().toString(); // 支付总金额
        String subject = alipayParam.getName(); // 订单名称
        String body = alipayParam.getIntoduction(); // 商品描述
